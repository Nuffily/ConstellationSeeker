...
chrono::high_resolution_clock::time_point commence_9;
...
*/
bool exit_flag_9 = false;
mpz_class min_checked_9("0");

void signal_handler_9(int signal) {
    exit_flag_9 = true;
}
...
void search_tuples_of_nine(const mpz_class& start, int thread_id, const vector<int>& hard_sieve,
    ...
    while (true) {
        ...
        if (is_candidate(checker, light_sieve)) {
            ...
            {
                lock_guard<mutex> lock(cout_mutex_9);
                ...
                std::chrono::duration<double> dur = std::chrono::high_resolution_clock::now() - commence_9;
    ...
    exit_l:
        ...
        if (iteration < offs_last)
            ...
            if (exit_flag_9) {
                ...
                if (min_checked_9 == 0 || min_checked_9 > current[0])
                    min_checked_9 = current[0];
...
int seekForNineTuples(mpz_class z) {
    ...
    signal(SIGINT, signal_handler_9);                         // Для перехвата CTRL + C

    commence_9 = chrono::high_resolution_clock::now();
    ...
    thread t1([&]() { search_tuples_of_nine(z, 1, super_sie, start_1, step_1, offs_1, 14, const_offs_1); });
    thread t2([&]() { search_tuples_of_nine(z, 2, super_sie, start_2, step_2, offs_2, 14, const_offs_2); });
    thread t3([&]() { search_tuples_of_nine(z, 3, super_sie, start_3, step_3, offs_3, 14, const_offs_3); });
    thread t4([&]() { search_tuples_of_nine(z, 4, super_sie, start_4, step_4, offs_4, 14, const_offs_4); });
    thread t5([&]() { search_tuples_of_nine(z, 5, super_sie, start_5, step_5, offs_5, 14, const_offs_5); });
    thread t6([&]() { search_tuples_of_nine(z, 6, super_sie, start_6, step_6, offs_6, 14, const_offs_6); });
