...
void search_quadruplets(const mpz_class& start, int thread_id, const vector<short>& hard_sieve,
    const short start_off, const short step, const vector<short>& offs, const int offs_last, const int* const_offset) {
    
    mpz_class current[9];
    current[0] = start + (step - start % step) + start_off;

    unsigned int prime_mul = 1;

    const vector<short> light_sieve = sieve(40, 14);  // Р РµС€РµС‚Рѕ
...
//                if (thread_id == 4)
//                    cout << current[i] << " " << i << " " << thread_id << endl;
...
int main() {
    ...
    const int const_offfs_2[8] = { 4, 6, 10, 16, 18, 24, 28, 30 };
    ...
    const int const_offfs_3[8] = { 2, 6, 12, 14, 20, 24, 26, 30 };
    ...
    const int const_offfs_4[8] = { 4, 10, 12, 18, 22, 24, 28, 30 };
